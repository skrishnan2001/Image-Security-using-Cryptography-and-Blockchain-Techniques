...
from Contract_Functions import web3_init, getNames, getSharedImages,shareImage,doesUserExist,grantAccess, revoke_access,registerUser
app = Flask(__name__)
...
def share(user,user2):
        ...
        returned_val = {'data':{"user1":user1,"user2":user_2,"url":url,"image_hash":image_hash,"time_stamp":time_stamp,"caption":caption}}
        try:
         shareImage(web3, contract, acct_private_key, user1, user_2, url, image_hash, time_stamp,caption)
...
def access_modifier(user):
     ...
     try:
        ...
        try:
         if(type == "grant"):
               result =  grantAccess(web3, contract, acct_private_key, receiver, sender,idx)
               print("on grant, result:",result)
               result = result['status']
         else:
             result = revoke_access(web3, contract, acct_private_key, sender, receiver,idx)
...
def register():
    ...
    try:
       registerUser(web3,contract,acct_private_key,userName,firstName,lastName)
